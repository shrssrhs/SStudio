...
class SceneObject:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "SceneObject":
        allowed = {
            ...
            "can_collide", "cast_shadow", "locked", "tags", "attributes",
        }
...
class EngineBridge(QObject):
    ...
    def __init__(self, objects: Optional[Iterable[SceneObject]] = None) -> None:
        super().__init__()
        self.objects: list[SceneObject] = list(objects or demo_scene())
    ...
    def set_adapter(self, adapter: Any) -> None:
        ...
        self._adapter_call("on_scene_replaced", self.objects)
        self.log("info", f"Engine adapter attached: {type(adapter).__name__}")

    def _adapter_call(self, method_name: str, *args: Any) -> None:
    ...
    def select(self, object_id: Optional[str]) -> None:
        ...
        self._adapter_call("on_object_selected", object_id)

    def add_part(self, object_type: str = "Part") -> SceneObject:
        defaults = {
            "Part": dict(size=Vec3(4, 4, 4), color="#a3a3a6"),
            "Mesh": dict(size=Vec3(5, 5, 5), color="#8fb8c9"),
            "UI": dict(size=Vec3(6, 3, 0.2), color="#7aa8d9", can_collide=False),
            "Light": dict(size=Vec3(1, 1, 1), color="#f2dc87", can_collide=False),
        }
        values = defaults.get(object_type, defaults["Part"])
        obj = SceneObject(
            name=self.unique_name(object_type),
        ...
        self.scene_changed.emit()
        self._adapter_call("on_object_added", obj)
        self.select(obj.id)
        self.set_dirty(True)
        self.log("info", f"{object_type} '{obj.name}' created.")
        return obj
    ...
    def duplicate_selected(self) -> Optional[SceneObject]:
        ...
        data["name"] = self.unique_name(source.name)
        data["position"]["x"] += 2.0
        data["position"]["z"] += 2.0
        ...
        self._adapter_call("on_object_added", duplicate)
    ...
    def delete_selected(self) -> bool:
        ...
        if obj is None:
            return False
        if obj.object_type == "Baseplate":
            self.log("warning", "Baseplate deletion is blocked in the prototype.")
            return False
        deleted_id = obj.id
        ...
        self.scene_changed.emit()
        self.selection_changed.emit(None)
        self._adapter_call("on_object_deleted", deleted_id)
        self.set_dirty(True)
    ...
    def set_property(self, object_id: str, property_path: str, value: Any) -> None:
        ...
        self._adapter_call("on_property_changed", object_id, property_path, value)
        self.set_dirty(True)
    ...
    def save_to_file(self, path: str | Path) -> None:
        ...
        self.log("info", f"Scene saved: {target.name}")
...
class Ribbon(QWidget):
    ...
    def _build_scene_page(self) -> QWidget:
        ...
        insert.add_widget(RibbonToolButton("Add", "plus", lambda: self.bridge.add_part("Part")))
        insert.add_widget(RibbonToolButton("Part", "cube", lambda: self.bridge.add_part("Part")))
        insert.add_widget(RibbonToolButton("Mesh", "mesh", lambda: self.bridge.add_part("Mesh")))
        insert.add_widget(RibbonToolButton("UI", "ui", lambda: self.bridge.add_part("UI")))
...
class ExplorerPanel(QWidget):
    def __init__(self, bridge: EngineBridge, parent: Optional[QWidget] = None) -> None:
        ...
        self.add_button.setToolTip("Add Part")
        self.add_button.clicked.connect(lambda: self.bridge.add_part("Part"))
    ...
    def rebuild(self) -> None:
        ...
        for obj in self.bridge.objects:
            path = [part for part in obj.parent.split("/") if part]
            if not path:
                path = ["Workspace"]
            ...
            if root is None:
                root = QTreeWidgetItem([root_name])
                root.setIcon(0, IconFactory.make("cube", 16))
                roots[root_name] = root
                self.tree.addTopLevelItem(root)
            ...
            for segment in path[1:]:
                ...
                folder = next(
                    (
                        ...
                        if parent_item.child(i).data(0, Qt.ItemDataRole.UserRole + 1) == accumulated
                    ),
                    None,
                )
                if folder is None:
                    folder = QTreeWidgetItem([segment])
                    folder.setData(0, Qt.ItemDataRole.UserRole + 1, accumulated)
                    folder.setIcon(0, IconFactory.make("cube", 16))
        ...
        for index in range(self.tree.topLevelItemCount()):
            ...
            self._expand_models(top)

        self._syncing = False
        self._sync_selection(self.bridge.get_object(self.bridge.selected_id))
        self._apply_filter(self.search.text())

    def _expand_models(self, item: QTreeWidgetItem) -> None:
        if item.text(0) in {"Models", "GreyBlock_B", "GreyBlock_C", "PinkMarkers"}:
            item.setExpanded(True)
        for i in range(item.childCount()):
            self._expand_models(item.child(i))
...
class InspectorPanel(QWidget):
    ...
    def set_object(self, obj: Optional[SceneObject]) -> None:
        ...
        pix = IconFactory.make("cube", 18, QColor(obj.color)).pixmap(18, 18)
        ...
        transform.add_row("Name", name_edit)
        ...
        self.contents_layout.insertWidget(self.contents_layout.count() - 1, transform)
        ...
        self.contents_layout.insertWidget(self.contents_layout.count() - 1, appearance)
        ...
        self.contents_layout.insertWidget(self.contents_layout.count() - 1, behavior)
        ...
        self.contents_layout.insertWidget(self.contents_layout.count() - 1, tags)
        ...
        self.contents_layout.insertWidget(self.contents_layout.count() - 1, attributes)
...
class MockViewport(QWidget):
    ...
    def _draw_objects(self, painter: QPainter) -> None:
        ...
        drawable = [
            obj for obj in self.bridge.objects
            if obj.object_type not in {"Camera", "Lighting", "Terrain", "Baseplate"}
        ]
...
class StudioMainWindow(QMainWindow):
    ...
    def _build_central(self) -> None:
        ...
        self.ribbon.tool_changed.connect(
            lambda tool: self.bridge.log("info", f"Active transform tool: {tool.title()}")
        )
